...
def custom_peak_loss(y_true, y_pred):
    def py_custom_peak_loss(y_true, y_pred):
        ...
        #return Nombre_de_succés / Nombre_de_données
...
def generate_and_save_prediction(epoch):
    ...
    # Génèrer une prédiction avec votre modèle sur un exemple de test
    key = 604
...
def train_step(model, inputs, targets, optimizer, loss_fn):
    ...
    # R�alise la descente de gradient et update le modèle
...
for epoch in range(epochs):
      ...
      # On d�termine si le success rate est meilleur que les époch précédentes et on sauvegarde la meilleur époch
